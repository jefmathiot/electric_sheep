...
describe ElectricSheep::Helpers::Directories do
  ...
  describe 'expanding paths' do
    it 'raises unless job directory created' do
      -> { directories.expand_path('some-file') }
        .must_raise RuntimeError, /job directory has not been created/
    end

    [nil, '/host/working/dir'].each do |working_dir|
      describe "with#{working_dir ? '' : 'out'} " \
        'an explicit working directory' do
