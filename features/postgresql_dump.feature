Feature: PostgreSQL Dump
  ...
  Scenario: Create a dump
    Given I tell the sheep to work on job "postgresql-dump"
    Then a dump of the PostgreSQL database should have been created

  Scenario: Create a dump as a superuser
    Given I tell the sheep to work on job "postgresql-dump-sudo"
    Then a dump of the PostgreSQL database should have been created

  Scenario: Authentication error
    Given I tell the sheep to work on failing job "postgresql-dump-auth-fail"
    Then I am notified the pg_dump failed due to an authentication failure

  Scenario: Unknown database error
    Given I tell the sheep to work on failing job "postgresql-dump-unknown-db-fail"
