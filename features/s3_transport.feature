Feature: S3 transport
  ...
  Scenario: Move a file using S3 from the localhost to a remote bucket
    Given a local file
    And a remote bucket
    When I tell the sheep to work on job "s3-move-local-to-remote"
  ...
  Scenario: Copy a file using S3 from the localhost to a remote host
    Given a local file
    And a remote bucket
    When I tell the sheep to work on job "s3-copy-local-to-remote"
  ...
  Scenario: Move a file using S3 from a remote bucket to the localhost
    ...
    And a remote file
    When I tell the sheep to work on job "s3-move-remote-to-local"
  ...
  Scenario: Copy a file using S3 from a remote bucket to the localhost
    Given a remote bucket
    And a remote file
    When I tell the sheep to work on job "s3-copy-remote-to-local"
    Then the S3 object should have been copied to the localhost

  Scenario: fail using S3
    Given a remote bucket
    And I tell the sheep to work on failing job "s3-fail"
